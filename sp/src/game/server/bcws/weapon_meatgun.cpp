...
extern ConVar sk_plr_num_meatgun_pellets;
extern ConVar sk_plr_num_meatgun_pellets_double;
extern ConVar sk_npc_num_meatgun_pellets;


class CWeaponMeatgun : public CBaseHLCombatWeapon
{
	DECLARE_DATADESC();
public:
	DECLARE_CLASS(CWeaponMeatgun, CBaseHLCombatWeapon );
...
IMPLEMENT_SERVERCLASS_ST(CWeaponMeatgun, DT_WeaponMeatgun)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS( weapon_meatgun, CWeaponMeatgun);
PRECACHE_WEAPON_REGISTER(weapon_meatgun);

BEGIN_DATADESC(CWeaponMeatgun)
...
acttable_t	CWeaponMeatgun::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponMeatgun);

void CWeaponMeatgun::Precache( void )
{
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponMeatgun::FireNPCPrimaryAttack( CBaseCombatCharacter *pOperator, bool bUseWeaponAngles )
{
...
#ifdef MAPBASE
	pOperator->FireBullets( sk_npc_num_meatgun_pellets.GetInt(), vecShootOrigin, vecShootDir, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0 );
#else
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMeatgun::Operator_ForceNPCFire( CBaseCombatCharacter *pOperator, bool bSecondary )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponMeatgun::Operator_HandleAnimEvent( animevent_t *pEvent, CBaseCombatCharacter *pOperator )
{
	switch( pEvent->event )
	{
...
//-----------------------------------------------------------------------------
float CWeaponMeatgun::GetMinRestTime()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CWeaponMeatgun::GetMaxRestTime()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 1.5f;
...
//-----------------------------------------------------------------------------
float CWeaponMeatgun::GetFireRate()
{
	if( hl2_episodic.GetBool() && GetOwner() && GetOwner()->Classify() == CLASS_COMBINE )
	{
		return 0.8f;
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponMeatgun::StartReload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
bool CWeaponMeatgun::Reload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponMeatgun::FinishReload( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CWeaponMeatgun::FillClip( void )
{
	CBaseCombatCharacter *pOwner  = GetOwner();
...
//
//-----------------------------------------------------------------------------
void CWeaponMeatgun::DryFire( void )
{
	WeaponSound(EMPTY);
...
//
//-----------------------------------------------------------------------------
void CWeaponMeatgun::PrimaryAttack( void )
{
	...
	pPlayer->FireBullets( sk_plr_num_meatgun_pellets.GetInt(), vecSrc, vecAiming, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, true, true );
...
//
//-----------------------------------------------------------------------------
void CWeaponMeatgun::SecondaryAttack( void )
{
	// Only the player fires this way so we can cast
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	...
	// Fire the bullets
#ifdef MAPBASE
	pPlayer->FireBullets( sk_plr_num_meatgun_pellets_double.GetInt(), vecSrc, vecAiming, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, false, false );
#else
...
//-----------------------------------------------------------------------------
void CWeaponMeatgun::ItemPostFrame( void )
{
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponMeatgun::CWeaponMeatgun( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponMeatgun::ItemHolsterFrame( void )
{
	// Must be player held
