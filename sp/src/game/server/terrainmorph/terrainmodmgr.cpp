...
#include "cbase.h"
#include "terrainmorph/terrainmodmgr.h"
...
void TerrainMod_Add( TerrainModType type, const CTerrainModParams &params )
{
	...
	/*
	UserMessageBegin( everyone, "TerrainModAdd" );
	...
	*/
	...
	/*
	UserMessageBegin( initFilter, "TerrainModAdd" );
		EncodeTerrainMod( type, params.m_vCenter, params.m_vNormal, usRadius, params.m_vecMin, params.m_vecMax, params.m_flStrength, ucFlags );
	MessageEnd();
	*/
	...
	//Setup terrain for morphing
	//ITerrainMod* terrainmod = SetupTerrainMod(type, params);
	// Apply the morph.
	//ApplyTerrainMorph(terrainmod); //Сука он не работает, у него какой то символ неразрешонный

	DevMsg("You cant Morphing terrain because compile have LNK2019 Error (%i)\n", cantcount);
	cantcount++;
